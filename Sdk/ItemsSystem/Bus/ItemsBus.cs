...
namespace Vortex.Sdk.ItemsSystem.Bus
{
    ...
    /// и изменения его состава свойств. Слота сменяемого драйвера не имеет: подменять нечего, логика
    /// платформонезависимая. Понадобится — базу можно сменить аддитивно.
    ///
    /// Реактивность — не глобальным счётчиком, а событием предмета: изменение значения свойства или
    /// состава поднимает <see cref="ItemModel.OnChanged"/>, на которое подписывается инвентарь.
    /// Производные величины считаются по требованию и потому всегда актуальны.
    ///
    /// Статический класс, а не <c>Singleton</c>: экземпляр никуда не передаётся и интерфейсов не
    /// реализует, а сброс состояния через Dispose тут ни к чему.
    /// </summary>
    public static class ItemsBus
    {
        ...
        /// восстановит его целиком, вплоть до конкретных классов свойств — данные игрока не теряются
        /// из-за пропавшего пресета. Решение о судьбе такого предмета принимает держатель коллекции.
        /// </summary>
        ...
        /// назначению, но событие <see cref="OnItemCreated"/> не поднимается. Предмет транзиентный:
        /// он не входит ни в один инвентарь и не доходит до доменных подписчиков. Нужен для проверок
        /// вместимости и прочего осмотра, где полноценное построение дало бы фантомный предмет в
        /// чужой доменной логике.
        /// </summary>
