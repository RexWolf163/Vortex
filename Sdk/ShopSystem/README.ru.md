...
- Готовые логики оплаты: `FreeLogic` (нулевая цена), `LimitedLogic` (лимит количества за период)
...
- Композиция нескольких правил оплаты на одном товаре — у товара одна логика оплаты; «цена + лимит» реализуется одной комбинированной логикой
...
│           └── LimitedLogic.cs           # лимит количества за период
├── Presets/
