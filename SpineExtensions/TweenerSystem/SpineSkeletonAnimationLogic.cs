...
{
    /// <summary>
    /// Запускает анимацию спайна на <see cref="SkeletonAnimation"/> (MeshRenderer-вариант,
    /// в отличие от <see cref="SpineAnimationLogic"/>, который работает с <see cref="SkeletonGraphic"/>).
    ...
    /// </summary>
    [Serializable]
    public class SpineSkeletonAnimationLogic : TweenLogic
    {
        ...
        {
            var skeletonData = skeleton != null && skeleton.skeletonDataAsset != null
                ? skeleton.skeletonDataAsset.GetSkeletonData(true)
                : null;
            var list = skeletonData?.Animations.Select(a => a.Name).ToList() ?? new List<string>();
            list.Insert(0, None);
            return list.ToArray();
        }
