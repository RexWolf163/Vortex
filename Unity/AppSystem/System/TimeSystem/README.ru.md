...
**Вход:**
- Длительность (`float` секунд или `TimeSpan`) + callback `Action`

> `Timer` считает **длительность**, а не абсолютный момент. Для «сработать во вторник в 2:00» (календарная точка, в т.ч. оффлайн) — `DateTimeTimer` из `Vortex.Core.System.Abstractions.Timers`: у `Timer` пауза сдвигает конец на своё время, что дедлайн бы промахнуло.
...
- `Extend(bonus)` — продлевает активный таймер: сдвигает `End` и `Duration` на `bonus`, сохраняя уже прошедшее время (`Remains += bonus`, `GetTimePassed()` не «прыгает к максимуму»). No-op при `IsComplete` / после `Dispose` / `bonus <= 0`. На паузе двигает замороженный остаток вручную
...
// Продление активного таймера
timer.Extend(TimeSpan.FromSeconds(3));   // Remains += 3s, прошедшее время сохраняется
```
...
Секундомер (`IDisposable`): считает время **вверх** от старта до явного `Stop()` — зеркало `Timer` (тот считает вниз к дедлайну с колбэком). Опросный, как `Timer`/`DateTimeTimer`: реактивных полей нет намеренно — значение читает UI нужного секундомера, а не все активные тикают постоянно ради тех, на кого никто не смотрит. **Конструктор отсчёт не запускает.**

### Архитектура

```
Stopwatch (class)
├── Elapsed      — TimeSpan  (накопленное; на паузе и после Stop — заморожено)
├── TotalSeconds — int       (целые секунды Elapsed, для опроса из UI)
├── IsRunning    — bool      (идёт отсчёт)
├── IsPaused     — bool      (заморожен паузой — только в паузируемом режиме)
├── Start() / StartRealtime() — запуск: паузируемый / пауза-агностик
├── SetPause() / Resume()     — заморозка / возобновление (паузируемый режим)
├── Stop() / Reset()          — фиксация / обнуление
└── Dispose()                 — очистка (отписка от editor-паузы)
```

Никакого `TimeController`: `Elapsed` вычисляется по `DateTime.UtcNow` в момент чтения, срабатывать нечему.

### Контракт

**Вход:**
- Ничего в конструкторе. Запуск — `Start()` (паузируемый) или `StartRealtime()` (пауза-агностик).

**Выход:**
- `Elapsed` (`TimeSpan`), `TotalSeconds` (`int`), `IsRunning`, `IsPaused` — опросом в любой момент.

**Гарантии:**
- `Start()`/`StartRealtime()` начинают отсчёт **с нуля**; повторный старт идущего/стоящего на паузе — no-op.
- `Start()` — паузируемый: `SetPause`/`Resume` и пауза редактора замораживают накопление (меряется активное игровое время, без пауз).
- `StartRealtime()` — пауза-агностик: чистый wall-clock, `SetPause` и пауза редактора игнорируются (меряется полное реальное время, включая паузы).
- `SetPause` — no-op в непаузируемом режиме, для остановленного, уже на паузе или после `Dispose`.
- `Stop()` терминален: фиксирует `Elapsed`, дальше не растёт; возобновление невозможно (нужен `Reset` + `Start`).
- `Reset()` обнуляет и останавливает — готов к новому запуску.

**Ограничения:**
- `TotalSeconds` — целые секунды (усечение вниз). Точное значение — `Elapsed`.
- Editor-хук держит ссылку на паузируемый секундомер: не `Stop`/`Dispose`'нутый не соберётся GC до `Stop`/`Reset`/`Dispose`. В runtime-сборке хука нет. Пауза-агностичный режим не хукается — ограничения у него нет.
...
// Активное игровое время (пауза замораживает)
_watch.Start();

// В обработчике смены состояния — как у Timer
private void OnStateChanged(MiniGameStates state)
{
    switch (state)
    {
...
// Опрос из UI
label.text = $"{_watch.TotalSeconds} с";

_watch.Stop();                 // зафиксировать результат
var total = _watch.Elapsed;    // прочитать

// Полное реальное время (паузу считаем тоже)
_watch.StartRealtime();        // SetPause и пауза редактора на него не влияют
```
...
| Ситуация | Поведение |
|---|---|
| Чтение `Elapsed`/`TotalSeconds` до `Start` | `Zero` / `0` |
| Повторный `Start` идущего секундомера | no-op (не сбрасывает) |
| `SetPause` в режиме `StartRealtime` | no-op — считает сквозь паузу |
| `Resume` без предшествующего `SetPause` | no-op |
| `Start` после `Stop` | новый замер с нуля (прежнее значение теряется — прочитайте `Elapsed` до `Start`) |
| Уход в фон / пауза редактора (паузируемый) | накопление заморожено, простой не засчитан |
| Уход в фон (пауза-агностик) | время идёт — считается как реальное |
| `Dispose` во время editor-паузы | отписка снята, воскрешения на снятии паузы не будет |
