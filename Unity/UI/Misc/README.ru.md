...
Базовый компонент для отображения счётчика с минимумом/максимумом/текущим значением, набором слайдеров, пульсацией и пороговыми визуальными состояниями. Извлечение чисел из модели и подписка на её реактивные поля — на подклассе; база берёт на себя lifecycle, рендер и переключение состояний.

Наследник реализует:
- `int GetValue()` — текущее значение
- `int GetMinValue()` — минимум диапазона
- `int GetMaxValue()` — максимум диапазона
- `void Init()` — подписка на реактивные поля `Data`
- `void DeInit()` — симметричная отписка (должна быть идемпотентной: `-= handler` на пустом event — no-op, а обращения к внутренней ссылке на модель — под null-guard, потому что `DeInit` может быть вызван до первого `Init`)

Доступ к модели через `protected T Data`. Ссылка автоматически переустанавливается базой при `UpdateLink` (при полной смене содержимого storage'а).

**Lifecycle:**
- `OnEnable` → подписка на `IDataStorage.OnUpdateLink` + `switcher.Set(Empty)` + однократный `UpdateLink()` для первичной инициализации.
- `UpdateLink` (внутренний) при каждом сигнале от storage: `DeInit()` → перезабор `Data` → null-guard → синхронизация кэша значения → `Init()` → refresh всех блоков (min/max/value/switcher/sliders). Единый путь и для первого enable, и для последующих re-link'ов — подписки подкласса переезжают на актуальную модель без утечек.
- `OnDisable` → отписка от `OnUpdateLink` + `DeInit()`.
...
| — | `sliders` (`SliderView[]`) | Массив анимированных слайдеров — каждый обновляется теми же (value, max, min) |
...
API наследника:
- `UpdateValue()` / `UpdateMinValue()` / `UpdateMaxValue()` — подкласс дёргает эти методы из своей подписки на реактивное поле, чтобы протолкнуть refresh соответствующего блока UI.

### CounterViewAdvanced

Автономный компонент под типовой сценарий «три отдельных `IDataStorage`, в каждом лежит `IntData`». **Не наследник** `CounterViewBase<T>` — независимая реализация того же UI-поведения (те же пороги `CounterStates`, тот же pulse-tween, те же format-паттерны) с тремя раздельными источниками, самостоятельно подписывающимися на `IntData.OnUpdate` для value/min/max.

Особенности vs. `CounterViewBase<T>`:
- Три источника вместо одного (`sourceValue` / `sourceMin` / `sourceMax`) — удобно, когда min/max живут в отдельных моделях или свойствах.
- Один `SliderView` вместо массива.
- Не требует подкласса — работает out-of-the-box, если данные представлены как `IntData`.
- Ограничен `IntData` — для доменной модели с не-int полями или произвольным вычислением значений использовать `CounterViewBase<T>` с подклассом.
