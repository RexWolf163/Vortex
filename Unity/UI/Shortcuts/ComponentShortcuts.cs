...
namespace Vortex.Unity.UI.Shortcuts
{
    ...
    internal static class ComponentShortcuts
    {
        ...
        /// <summary>
        /// Добавить компонент отдельным слоем: в каждом выделенном объекте создаётся дочерний объект
        /// <c>[{тип}]</c> — первым в иерархии родителя, в нулевой точке (позиция и поворот 0, масштаб 1),
        /// на обычном Transform (RectTransform, если его накинет Unity, снимается), с компонентом. Повторное
        /// нажатие создаёт ещё один слой. Новые слои выделяются; отмена — одним шагом Undo.
        /// </summary>
