...
//
namespace Vortex.Steam.SteamConnectionSystem.Runtime
{
    ...
    public class SteamManager : MonoBehaviour
    {
        ...
        protected Callback<UserStatsReceived_t> _userStatsCallback;
        ...
        private static void Init() => s_instance = new GameObject("SteamManager").AddComponent<SteamManager>();
        ...
        {
            SteamBus.IsInitialized = false;
            SteamBus.IsLoaded = false;
            s_instance = null;
        }
#endif
        ...
        protected virtual void Awake()
        {
            ...
            try
            {
                ...
                if (!settings.IsTestBuild && SteamAPI.RestartAppIfNecessary(new AppId_t(settings.SteamAppId)))
                {
                    Debug.Log(
...
[*] Your App ID is not completely set up, i.e. in Release State: Unavailable, or it's missing default packages.",
            ...
            _userStatsCallback = Callback<UserStatsReceived_t>.Create(OnUpdatedStats);
            SteamBus.IsInitialized = true;
            LoadStatsInBus();
        }

        private void OnUpdatedStats(UserStatsReceived_t param)
        {
            SteamBus.User.CallOnUpdated();
        }

        private void LoadStatsInBus()
        {
            try
            {
                SteamBus.LoadServices();
