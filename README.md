...
При этом, если добавить переменной $data параметр _state, то его значение будет применено к ви-фрейму в качестве класса.
Например если $data->_state = "test", то мы получим в html <vframe vid="..." mid="..." class="test">
