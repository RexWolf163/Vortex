...
| `PlayScript(path, beforeLoad, token)` | Единая точка запуска скрипта под локом — см. ниже |
| `IsStartingScript` | `true` пока шлюз `PlayScript` захвачен (идёт `Load`, `Play` ещё не вызван) |
| `NaniIsPlaying()` | `true` если **идёт запуск через `PlayScript`**, ScriptPlayer играет **или видим хэндлер выбора** (обе дополнительные проверки несущие — см. ниже) |

### PlayScript — шлюз запуска

Запускать скрипт следует **только** через `NaniWrapper.PlayScript`, а не `ScriptPlayer.LoadAndPlay` напрямую.

`LoadAndPlay` — это `await IScriptLoader.Load(path)` плюс `ScriptPlayer.Play(path)`, и между ними асинхронное окно: внутри `Load` идёт preload акторов (инстанцирование префабов, загрузка аппиренсов) — десятки кадров. В этом окне `ScriptPlayer.Playing` ещё `false`, то есть плеер выглядит свободным. При `ResourcePolicy.Optimistic` очередной `Load` начинается с `UnloadAll()`, который отпускает держателей ассета, уже подготовленного первым вызовом. Первый доходит до `Play` и падает на `Failed to get '<path>' resource of type 'Naninovel.Script': not loaded`.

`PlayScript` сериализует запуски: пока один вызов не прошёл путь `Load → Play`, остальные ждут на шлюзе. Флаг шлюза учтён в `NaniIsPlaying()`, поэтому ждущие «пока Нани освободится» (например `RunNaniScript`) не проскакивают в окно. Шлюз снимается и на исключении — упавшая загрузка не блокирует последующие запуски.

`beforeLoad` выполняется **после** захвата шлюза и **до** `Load` — туда выносится подготовка, которая вне лока порвала бы чужой цикл (типовой случай: `ScriptPlayer.ResetService()` у галереи). `token` отменяет только ожидание шлюза; уже начатый `Load` не прерывается.

**Границы.** Лок держит лишь запуски, идущие через `PlayScript`. Синхронные сбросы плеера мимо него — `OnLoadGame` (`ScriptPlayer.ResetService()`) и `OnStateChanged` на `Off/Win/Fail` — шлюз не ждут: загрузка сейва или выход из сессии ровно в окне `Load→Play` по-прежнему способны оборвать запуск.
...
- **Проверка на `IsStartingScript` в `NaniIsPlaying()`** закрывает третий многокадровый случай — окно между `Load` и `Play` (см. `PlayScript`). Побочный эффект по контракту событий: пока шлюз занят, `OnNaniStop` не выстрелит, потому что отложенное подтверждение останова тоже опирается на `NaniIsPlaying()`. По смыслу верно (смена скрипта — не останов), но сдвигает тайминг подписчиков.
