...
- `SaveReactor` — блок коррекции устаревшего сейва: правка сырой строки до разбора в окне версий
...
3. `Driver.Load(guid)` — драйвер читает тело сейва, прогоняет реакторы коррекции (см. ниже) и заполняет `SaveDataIndex`
...
## Реакторы коррекции сейвов

Механизм правки устаревших сейвов. Формат данных знает модуль, а не система сохранений, поэтому Core даёт только шов: точку вызова, порядок и правило применимости. Конкретные правки пишет владелец данных.

### Как работает

Драйвер слота при загрузке распаковывает тело сейва и **до разбора** отдаёт строку в `SaveReactors.Apply(raw, saveVersion, reactors)`. Реакторы применяются по порядку списка, каждый возвращает строку после своих правок; результат уходит в разбор.

Правка по сырой строке выбрана осознанно: после разбора чинить нечего — сборка уже не прочитает снятый тип или переименованное поле. Типизированной стадии в системе сохранений нет: `SavePreset` состоит из строк, в объекты их разбирают сами модули.

### Применимость

Окно задаётся версией сборки, которой записан сейв (`SaveSummary.Version`): реактор применяется, когда версия не старше `minVersion` и не новее `maxVersion`. Пустая граница — без ограничения с этой стороны.

Сравнение (`SaveVersion.Compare`) — посегментно по точке, число сегментов произвольное. Из сегмента берётся числовой префикс до первого нецифрового символа (`260910-rc1` → `260910`), недостающий сегмент считается нулём (`1.0` = `1.0.0`). Пустая версия — самая старая: так читаются сейвы, записанные до появления поля в сводке.
...
Реактор добавляется в список `SaveSettings` (Unity-слой), там же задаются границы окна. Список по умолчанию пуст.

### Гарантии и ограничения

- Сейв на диске **не переписывается**: коррекция живёт только в загруженных данных. Версия в сводке меняется при следующем сохранении.
- Коррекция выполняется на каждой загрузке — результат не кешируется.
- Сейв, не попавший ни в одно окно (в том числе более новый, чем сборка), читается как есть.
- Реактор не знает о других реакторах и о структуре чужих модулей — нужный кусок он находит в строке сам.
- Исключение внутри реактора прекращает коррекцию: в лог уходит имя блока и стек, разбор идёт по строке, полученной до сбойного блока.
- Глобальное хранилище механизмом не охвачено: у `GlobalContainer` версии нет, формат данных там — забота модуля.
