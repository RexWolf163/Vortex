...
Оба кеша заполняются при первом обращении и не очищаются. Защита от циклов — **локальный** `HashSet<object>`: создаётся на каждый вызов `SerializeProperties` и протаскивается по рекурсии. Поэтому сериализация re-entrant (вложенный вызов из custom-конвертера не портит внешний проход) и потокобезопасна.
...
| Ключи словарей — простые типы или `Type` | Сложные ключи не поддерживаются (лог + мягкая деградация `"null"`, не исключение) |
...
| Глубина десериализации/загрузки ≤ `MaxDepth` (64) | Страховка от битого глубоко-вложенного сейва: превышение → лог + `null`, а не StackOverflow |
