    ...
    public static partial class SerializeController
    {
        ...
        private static void UploadCollection<T>(T target, string data, int depth = 0)
        {
            ...
            if (string.IsNullOrWhiteSpace(data))
                return; // пустая коллекция ([] / только пробелы) — элементов нет
            var ar = SeparateText(data);
            foreach (var s in ar)
            {
        ...
        private static void UploadDictionary<T>(T target, string data, int depth = 0)
        {
            ...
            data = TrimData(data);
            // Пустой словарь ({} / только пробелы) — записей нет. Без этого SeparateText вернул бы один
        ...
        private static void UploadArray<T>(ref T target, string data, int depth = 0)
        {
            ...
            if (string.IsNullOrWhiteSpace(data))
            {
                target = (T)(object)Array.CreateInstance(elementType, 0); // пустой массив ([] / только пробелы)
                return;
            }
