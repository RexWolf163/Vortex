...
`SimpleTypeMarker` — partial-класс. В Core пуст. Unity-partial добавляет типы (например `UnityEngine.Object`), которые не должны клонироваться — передаются по ссылке. Проверка через `IsAssignableFrom` — наследники маркерных типов тоже считаются платформенными примитивами. Типы кешируются при первом вызове через рефлексию по статическим полям класса.
